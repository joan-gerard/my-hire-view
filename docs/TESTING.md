...
| `__tests__/unit/api/applications-create.test.ts` | **Flow #4 — Create application** — `POST /api/applications`: 201, profile fallback for candidate fields, `show_profile_picture` preference (no stored picture URL), DB insert failure → 400, 429, 401 |
