...
| `__tests__/unit/lib/auth/safe-next-path.test.ts` | **Auth callback redirect sanitizer** — allows same-origin relative paths; rejects `//…` and backslash open-redirect tricks (`/\evil.com`) |
