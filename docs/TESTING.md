...
| `__tests__/unit/api/applications-list.test.ts` | **Dashboard list** — `GET /api/applications`: default limit 20 + `meta.total`, custom `limit`/`offset`, max limit cap, `q` search filter (strips quotes/reserved chars, keeps apostrophes, skips empty-after-strip), `cv_exists` true for unknown/non-R2 and false when missing, 401, 429, 500 |
