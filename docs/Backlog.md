...
| L11-114 | Infrastructure | Orphan profile-picture cleanup cron + drop delete warnings | After F9: scheduled job deletes Storage objects in `profile-pictures` that are not the committed `profiles.profile_picture_url` (keep a short grace window for in-flight uploads). Then **stop surfacing** failed previous-picture delete / folder-sweep messages to users (`warnings` → UI); keep server logs. New photo already works without user action. Distinct from tailored-CV orphans `L3-084`. | [PROFILE_PICTURE.md](PROFILE_PICTURE.md)                              |
...
| L11 | `feat/l11-avatar-orphan-cleanup`      | `L11-114`                    | Orphan profile-picture cleanup cron; stop showing old-picture delete warnings to users (after F9)               |
