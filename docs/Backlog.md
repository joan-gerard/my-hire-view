...
| F16-050 | Product     | Preview before submitting an application                  | Save as `status = draft`, owner preview on the share URL, then publish to `active`. **`status` / `archived_at` shipped**; remaining work is this preview/publish UX (in progress on `feat/f16-application-preview-draft`). Relates to local form-draft persistence (`F15`).                                                                                                                                       | [CV_REUSE_AND_STORAGE.md](CV_REUSE_AND_STORAGE.md)                     |
...
| F16 | `feat/f16-application-preview-draft`     | `F16-050`                       | Preview / draft-before-publish UX (after F15) — in progress                                                        |
...
1. Continue the phased sequence in [BEFORE_LAUNCH_PR_ORDER.md](BEFORE_LAUNCH_PR_ORDER.md) — next after F16: **F19** (onboarding + Public id + legend)  
