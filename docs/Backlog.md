...
| B3-042 | Security | DB same-user ownership for `primary_cv_id` | API checks ownership; FK only references `primary_cvs(id)`. **In progress:** forward migration `027` — same-user trigger (not composite FK: `ON DELETE SET NULL` on `(primary_cv_id, user_id)` would also null `user_id`). Keep id-only FK + `ON DELETE SET NULL`. | [API_REFERENCE.md](API_REFERENCE.md) |
