...
| B3-042 | Security | DB same-user ownership for `primary_cv_id` | API checks ownership; FK only references `primary_cvs(id)`. Forward migration: composite ownership constraint/trigger (keep `ON DELETE SET NULL`). Detail in POST/PUT deferred tables. | [API_REFERENCE.md](API_REFERENCE.md) |
