...
| ------ | -------------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| E1-012 | Product        | Pricing & membership tiers                    | **Defer until near public launch** — plans/prices/caps may still change. Working draft lives in [PRICING_AND_MEMBERSHIP.md](PRICING_AND_MEMBERSHIP.md) and on the homepage `#pricing` section (E3). When ready: lock Free/Pro/Premium, per-tier limits, and price points; do **not** launch with unlimited free app access. Must finish before E2.                                                                        | [PRICING_AND_MEMBERSHIP.md](PRICING_AND_MEMBERSHIP.md) |
...
| ID      | Subcategory | Item                                                      | Notes                                                                                                                                                                                                                                                                                                                                                                                                                      | Source                                                                 |
| ------- | ----------- | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| F17-017 | Branding    | Update `/admin` and `/view` branding                      | Align with homepage warm palette (`app/globals.css`): loading skeletons, status icon colors, card chrome, primary/secondary/accent buttons (visual skin only — action hierarchy / legend copy shipped in `F19-047`).                                                                                                                                                                  | [PROJECT_OVERVIEW.md](PROJECT_OVERVIEW.md)                             |
| F17-018 | Branding    | Update login and signup branding                          | Match `/login` and `/signup` to homepage visual identity (logo shell, warm surface panel, spaced inputs, accent focus rings), including loading states.                                                                                                                                                                                                                                                                                                | —                                                                      |
| F20-019 | Legal       | Create legal pages                                        | Terms of Service, Privacy Policy, and Cookies (`/terms`, `/privacy`, `/cookies`). Marketing footer and `/view` footer already link to these paths with no pages yet. **Context:** the marketing homepage was built from a **Framer template that was modified** — keep that origin in mind for layout/styling consistency with marketing, and for any license/attribution obligations the template requires when drafting legal copy. | —                                                                      |
...
| F22-021 | Docs / DX   | Refresh README                                            | Still create-next-app boilerplate above the MyHireView section. Migrations setup step now points at applying **all** files in `supabase/migrations/` in order (through `028`); finish the full README rewrite.                                                                                                                                                                                                             | [PROJECT_OVERVIEW.md](PROJECT_OVERVIEW.md)                             |
...
| ~~F19-044~~ | ~~UX~~          | ~~New-user onboarding checklist~~                             | ~~Shipped: floating Getting started checklist on all `/admin` routes (create profile, add location or a link, photo, primary CV, first app, publish/share); minimizable; per-step Skip + Skip all; stays visible when complete/skipped until **Dismiss**; prefs in one localStorage blob scoped by account key; initial snapshot from admin layout (no four `/api` GETs on first paint); client refetch on mutation notify only. Complementary guided tour after launch: `F31-105`.~~                                                                                                                                                                                                                              | —                                                                      |
| ~~F19-046~~ | ~~UX~~          | ~~Explain Public id + show final URL~~                        | ~~Shipped: profile Account shows Public id + help; create/edit share link preview under Slug (`F19-046`).~~                                                                                                              | [PUBLIC_URL_OPTION_B.md](PUBLIC_URL_OPTION_B.md)                       |
| ~~F19-047~~ | ~~UX~~          | ~~Application card icon / badge legend~~                      | ~~Shipped: dashboard “What do these icons mean?” button opens a modal legend (status icons + draft actions). Brand colors/chrome for cards remain `F17-017`.~~                                                                                                                                                                                          | —                                                                      |
...
| ID      | Subcategory    | Item                                                 | Notes                                                                                                                                                                                                                                                                                                                                          | Source                                                                         |
| ------- | -------------- | ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| I2-078  | Code quality   | Single source of truth for DB types                  | Derive/generate from schema; stop dual `application` / `database` shapes. **Pulled before launch** as a prerequisite for `F30-104`.                                                                                                                                                                                                            | [CODE_REVIEW.md](CODE_REVIEW.md)                                               |
| L6-088  | Code quality   | Central API client                                   | Wrap client `fetch` with defaults / future interceptors. **Pulled before launch** as a prerequisite for `F30-104`.                                                                                                                                                                                                                             | [CODE_REVIEW.md](CODE_REVIEW.md)                                               |
...
| L11-114 | Infrastructure | Orphan profile-picture cleanup cron + drop delete warnings | After F9: (1) scheduled job deletes Storage objects in `profile-pictures` that are not the committed `profiles.profile_picture_url` (short grace window for in-flight uploads); (2) **stop surfacing** failed previous-picture delete / folder-sweep messages to users (`warnings` → UI), keep server logs; (3) harden cross-instance picture-PUT races — F9’s in-memory per-user slot is best-effort on one Next.js instance only; overlapping picture changes on different instances can still purge each other’s committed avatar — use a distributed lock (e.g. Upstash) and/or versioned cleanup that only deletes when DB still matches the committed URL. Distinct from tailored-CV orphans `L3-084`. | [PROFILE_PICTURE.md](PROFILE_PICTURE.md)                              |
...
| L8-096  | API            | Upload metrics / correlation                   | Duration, size, idempotent/purge-warning rates, 5xx; optional request id in logs. Post-launch if support volume warrants (CV + profile-picture uploads). Complements error tracking `P1-113` (request id can join Vercel/Sentry).                                                                                                                      | [API_REFERENCE.md](API_REFERENCE.md)                                  |
...
| F31-105 | UX             | New-user product tour                          | Guided intro to `/admin` navigation and creating an application. **Brainstorm formats first** (modal / multi-step dialog, spotlight coach-marks, short video, empty-state walkthrough, replayable Help entry). Dismissible/skippable — complementary to checklist `F19-044`, not a duplicate. [GO_TO_MARKET.md](GO_TO_MARKET.md) notes tours are not the aha (first shared link is). | [GO_TO_MARKET.md](GO_TO_MARKET.md)                                    |
| N0-109  | Design         | Brainstorm & design public-view personalization | Ideas + lightweight mockups for a more personal `/view` (layouts, colors, headline/quotes/photos) **before** coding `N2`/`N3`. Can run with or after product scope `N1-106`. Deliver a short design note / sketches N2–N3 can implement against.                                                                                                                              | [PRICING_AND_MEMBERSHIP.md](PRICING_AND_MEMBERSHIP.md)                 |
| N1-106  | Product        | Scope public-view personalization (Premium?)   | Decide which personality / branding knobs belong on `/view` (headline, quotes, accent colors/theme, extra photos/gallery, etc.), Free vs Pro vs **Premium** gating, and per-application vs profile defaults. Update [PRICING_AND_MEMBERSHIP.md](PRICING_AND_MEMBERSHIP.md). **Prerequisite for N2/N3** (with `N0-109`). Needs billing (`E2`) before Premium gate ships. | [PRICING_AND_MEMBERSHIP.md](PRICING_AND_MEMBERSHIP.md)                 |
| N2-107  | Product        | Public view theme / accent colors              | Let candidates customize `/view` look (accent/theme colors, maybe background) so pages feel personal — not just the default brand chrome. After `N0-109` + `N1-106`; prefer **Premium**. Schema + create/edit UI + public render.                                                                                                                          | [PRICING_AND_MEMBERSHIP.md](PRICING_AND_MEMBERSHIP.md)                 |
| N3-108  | Product        | Headline, quotes, and extra photos on `/view`  | Expand beyond name / profile picture / video / CV: optional headline, quote(s), and additional photos (gallery). Storage, validation, create/edit toggles, public layout. After `N0-109` + `N1-106`; prefer **Premium**. Complements vanity id `J1-079` as Premium branding.                                                                                       | [PRICING_AND_MEMBERSHIP.md](PRICING_AND_MEMBERSHIP.md)                 |
| O1-110  | Product        | Decide post-launch user intent capture         | Keep collecting waitlist-style signals (`job_search_status`, `primary_goal`, `career_stage`) after launch for marketing/product insight. Decide surface(s): signup (required/optional), optional `/admin/profile`, onboarding nudge (`F19-044`), or mix. Privacy/copy + enum continuity. **Prerequisite for O2/O3.** | [GO_TO_MARKET.md](GO_TO_MARKET.md)                                    |
| O2-111  | API            | Persist intent fields on profiles              | Store status / goal / career stage on the user (likely `profiles`); share/reuse waitlist Zod enums; wire profile (and signup if O1 chooses it) read/write. After `O1-110`.                                                                                                                                                                              | [GO_TO_MARKET.md](GO_TO_MARKET.md)                                    |
| O3-112  | UX             | Intent capture UI (signup and/or profile)      | UI for the O1-chosen surface(s). Low friction; don’t block creating applications. After `O1-110` + `O2-111`.                                                                                                                                                                                                                                            | [GO_TO_MARKET.md](GO_TO_MARKET.md)                                    |
| P1-113  | Infrastructure | Error tracking (Sentry)                        | Add Sentry (or equivalent) when support volume makes Vercel Runtime Logs too shallow. Wire unexpected API 500s through `handleApiError` (F5); optionally client/UI errors. No CV bytes, tokens, or emails. Complements `L8-096` (request id). Do not start with Datadog/self-hosted logs. **Post-launch** if real users warrant it.                      | —                                                                     |
...
| D3  | `feat/d3-durable-rate-limiting`       | `D3-001`           | ~~Durable rate limiting (Redis/Upstash) + per-path Map growth / interim validate-before-key~~ (shipped) |

#### Sprint E — Monetization (Must epic)

**Timing:** E3 homepage `#pricing` section is shipped. **E1 then E2 stay deferred until near public launch** — treat current prices/caps as a working draft that may change. Same “just before launch” bucket as `A3-015`.
...
| E3  | `feat/e3-pricing-page`       | `E3-014` | ~~Homepage `#pricing` section with draft Free/Pro/Premium~~ (shipped; no separate `/pricing` route; **re-align copy when E1 locks**, then wire checkout in E2) |
...
| PR  | Branch                                   | Tickets                         | Scope                                                                                                              |
| --- | ---------------------------------------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| F1  | `fix/f1-harden-auth-signup`              | `F1-040`, `F1-041`              | ~~Harden auth login/signup + enforce signup password rules~~ (shipped)                                 |
| F2  | `refactor/f2-auth-pages-dry`             | `F2-057`, `F2-059`              | ~~Login/signup DRY + show-password toggle~~ (shipped)                                                              |
| F3  | `feat/f3-waitlist-bot-protection`        | `F3-039`, `F3-064`              | ~~Waitlist honeypot + name/email validation (no CAPTCHA)~~ (shipped)                                       |
| F4  | `refactor/f4-with-auth-helper`           | `F4-025`                        | ~~Shared `withAuth` helper~~ (shipped)                                                                             |
| F5  | `refactor/f5-upload-handle-api-error`    | `F5-056`                        | ~~Upload routes → `handleApiError`~~ (shipped)                                                                     |
| F6  | `feat/f6-schema-validation-writes`       | `F6-024`                        | ~~Schema validation on remaining write routes~~ (shipped — primary CV label + delete `id`)                                                           |
| F7  | `fix/f7-upload-auth-and-replay`          | `F7-035`, `F7-036`              | ~~Auth before R2 config probe; stronger CV upload replay identity~~ (shipped)                                      |
| F8  | `feat/f8-upload-form-ux`                 | `F8-051`, `F8-055`, `F8-063`    | ~~Upload form errors / busy copy / progress / retry idempotency~~ (shipped)                                        |
| F9  | `fix/f9-avatar-purge-and-warnings`       | `F9-037`, `F9-052`, `F9-062`    | ~~Defer avatar purge until after URL commit; surface PUT warnings; concurrent-safe folder sweep~~ (shipped)        |
| F10 | `fix/f10-public-resolve-cv-exists`       | `F10-027`, `F10-029`, `F10-030` | ~~Resolve DB → 5xx; HeadObject vs missing CV~~ (shipped in D1 follow-up); ~~harden `toPublicApplication`~~ (shipped) |
| F11 | `fix/f11-escape-list-search-q`           | `F11-028`                       | ~~Escape `q` quotes for list search~~ (shipped)                                                                    |
| F12 | `fix/f12-repairable-auth-name-sync`      | `F12-031`                       | ~~Repairable Auth name sync on PUT profile~~ (shipped)                                                             |
| F13 | `fix/f13-atomic-primary-cv-cap`          | `F13-032`                       | ~~Atomic primary-library cap~~ (shipped — migration `028` + API maps trigger)                                      |
| F14 | `fix/f14-slug-name-clamp-fallback`       | `F14-033`, `F14-048`, `F14-099` | ~~Preserve name when clamping slugs; remove misleading create-slug auto-assign copy (keep Save blocked); edit form must keep saved custom slug on load~~ (shipped) |
| F15 | `feat/f15-create-app-draft-cv-mode`      | `F15-045`, `F15-049`            | ~~Persist create-application draft + preserve CV source while library loads~~ (shipped)                            |
| F16 | `feat/f16-application-preview-draft`     | `F16-050`                       | ~~Preview / draft-before-publish UX (after F15)~~ (shipped)                                                        |
| F17 | `feat/f17-admin-view-auth-branding`      | `F17-017`, `F17-018`            | Brand `/admin`, `/view`, login, signup (incl. dashboard card visual skin)                                          |
| F18 | `fix/f18-primary-cv-warning-draft-count` | `F18-053`, `F18-054`            | ~~Primary-CV delete warning sticks; profile apps summary includes drafts~~ (shipped)                               |
| F19 | `feat/f19-onboarding-public-id-legend`   | ~~`F19-044`~~, ~~`F19-046`~~, ~~`F19-047`~~ | ~~Onboarding checklist; Public id + URL copy; card icon legend (+ draft action clarity after F16)~~                    |
| F20 | `feat/f20-legal-pages`                   | `F20-019`                       | Legal pages (Terms / Privacy / Cookies). Marketing homepage originated from a modified Framer template — match that context for styling and any attribution/license needs. |
...
| F25 | `fix/f25-confirm-dialog-cancel`          | `F25-058`                       | ~~ConfirmDialog single cancel path~~ (shipped)                                                                     |
...
| PR  | Branch                                 | Tickets            | Scope                                                                          |
| --- | -------------------------------------- | ------------------ | ------------------------------------------------------------------------------ |
| G1  | `feat/g1-status-archived-at-invariant` | _(prerequisite)_   | DB `status` ↔ `archived_at` invariant (CHECK/trigger) — called out on `G2-072` |
...
| I2  | `refactor/i2-db-types-single-source` | `I2-078`           | ~~Moved to before-launch F-batch (prerequisite for `F30-104`)~~   |
...
| L11 | `feat/l11-avatar-orphan-cleanup`      | `L11-114`                    | Orphan avatar cron; drop delete warnings; distributed/versioned picture-PUT cleanup (after F9)                  |
...
| L6  | `refactor/l6-central-api-client`      | `L6-088`                     | ~~Moved to before-launch F-batch (prerequisite for `F30-104`)~~                                                 |
...
#### Epic F31 — New-user product tour (nice-to-have)

| PR  | Branch                           | Tickets   | Scope                                                                                         |
| --- | -------------------------------- | --------- | --------------------------------------------------------------------------------------------- |
| F31 | `feat/f31-new-user-product-tour` | `F31-105` | Product tour after launch (brainstorm modal/video/coach-marks first); relates to `F19-044`    |

#### Epic N — Public view personalization (Premium-leaning)

Depends on design brainstorm (`N0`) + product scope (`N1`); Premium gate needs `E2`. Ship after core `/view` branding (`F17`) if still open.

| PR  | Branch                                    | Tickets  | Scope                                                                                                      |
| --- | ----------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
| N0  | `docs/n0-public-view-design-brainstorm`   | `N0-109` | Brainstorm ideas + lightweight mockups / design note **before** coding N2/N3                               |
| N1  | `docs/n1-public-view-personalization`     | `N1-106` | Scope knobs + Free/Pro/Premium gating; update [PRICING_AND_MEMBERSHIP.md](PRICING_AND_MEMBERSHIP.md)        |
| N2  | `feat/n2-public-view-theme-colors`        | `N2-107` | Theme / accent colors on `/view` (after N0 + N1; prefer Premium)                                           |
| N3  | `feat/n3-public-view-personality-content` | `N3-108` | Headline, quotes, extra photos on `/view` (after N0 + N1; prefer Premium)                                  |

#### Epic O — Post-launch user intent capture (marketing insight)

When the waitlist is no longer the main intake, keep learning job-search status / primary goal / career stage from signed-in users so marketing and product stay tailored ([GO_TO_MARKET.md](GO_TO_MARKET.md)).

| PR  | Branch                          | Tickets  | Scope                                                                                          |
| --- | ------------------------------- | -------- | ---------------------------------------------------------------------------------------------- |
| O1  | `docs/o1-user-intent-capture`   | `O1-110` | Decide capture surface(s): signup vs optional profile vs onboarding (or mix) + privacy/copy    |
| O2  | `feat/o2-profile-intent-fields` | `O2-111` | Persist intent fields on profiles; reuse waitlist enums (after O1)                             |
| O3  | `feat/o3-intent-capture-ui`     | `O3-112` | Signup and/or profile UI for intent fields (after O1 + O2)                                     |

#### Epic P — Error tracking (after launch)

First observability step when Vercel Runtime Logs are no longer enough. Upload-specific metrics stay in `L8-096`.

| PR  | Branch                            | Tickets  | Scope                                                                                          |
| --- | --------------------------------- | -------- | ---------------------------------------------------------------------------------------------- |
| P1  | `feat/p1-sentry-error-tracking`   | `P1-113` | Sentry (or equivalent) via `handleApiError`; optional client errors; no PII                    |
...
| `F3-039`  | **Won’t (CAPTCHA / Turnstile)** — shipped honeypot + rate limit + validation instead; revisit if waitlist spam becomes real |
...
**Near launch (not now):** **E1 → E2** — `docs/e1-pricing-tiers` → `feat/e2-payment-membership` (plus `A3-015`). Keep the working draft in [PRICING_AND_MEMBERSHIP.md](PRICING_AND_MEMBERSHIP.md) and the homepage `#pricing` section updated if product thinking changes, but do not lock or build billing until launch is imminent.
