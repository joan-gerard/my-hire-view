...
- **Attach check:** Creating/updating a tailored application CV rejects `cv_url` values that are not under `cvs/{userId}/tailored/…`, stores a **canonical** public URL (pathname only; per-segment encoding; no search/hash), and rejects URLs already used by another of the user’s applications (**409**, object-key compare with pagination past PostgREST row limits). Partial unique index `applications_user_id_tailored_cv_url_key` backs races. Re-uploading the same PDF for a second app creates a distinct object. Primary CVs remain intentionally shareable via `cv_type: "primary"`.
