...
- **Attach check:** Creating/updating a tailored application CV rejects `cv_url` values that are not under `cvs/{userId}/tailored/…`, stores a **canonical** public URL (pathname only; per-segment encoding; no search/hash), and rejects URLs already used by another of the user’s applications (**409**: exact canonical match, then keyset-paginated object-key scan for legacy variants). Partial unique index `applications_user_id_tailored_cv_url_key` backs races. Re-uploading the same PDF for a second app creates a distinct object. Primary CVs remain intentionally shareable via `cv_type: "primary"`.
...
Objects are uploaded with `Cache-Control: public, max-age=31536000, immutable`. **Do not overwrite an existing key** after changing the video: browsers keep the old bytes for up to a year. Upload a new key (version suffix or content hash), update `marketingAssetUrl("…")` in `components/public/how-it-works/constants.ts` (homepage step videos) to match, then deploy. Optionally delete the old object later.
