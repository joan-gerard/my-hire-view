...
5. Progress on this form is **remembered in this browser** if you refresh or leave and come back (up to about a week). Drafts are stored per signed-in account (not a shared browser key). After a successful Save, the draft is cleared; a failed Save keeps it. A tailored PDF file itself is not restored — re-choose the file if you had one selected.
