...
- **Tailored CV:** when `cv_type` is `"tailored"` (default), `cv_url` must be a caller-owned tailored upload (`cvs/{userId}/tailored/…`); not a primary library key. The server stores a **canonical** public URL (decoded object key under `R2_PUBLIC_BASE_URL`). Reusing another application’s tailored object (same key / URL, including percent-encoded variants) returns **409**. Uniqueness is checked by object key before insert; Postgres partial unique index `applications_user_id_tailored_cv_url_key` (`user_id`, `cv_url` where `cv_type = 'tailored'`) is the race backstop.
