...
- **Client Save UX (F8):** `ApplicationForm` maps upload **400** / **401** / **409** / **429** / **5xx** (and network failures) via `messageForUploadFailure` / `messageForUploadNetworkError`; Save shows **Uploading…** during `POST /api/upload` then **Saving…**; the same idempotency key is reused on retry until the selected file changes (`resolveCvUploadIdempotencyKey` — F8-063).
...
- **Client Save UX (F8):** `ProfileForm` and `ProfilePictureModal` map upload failures the same way as CV upload; Save shows **Uploading…** during `POST /api/upload/profile-picture`, then **Saving…**.
