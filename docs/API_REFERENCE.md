...
- Omitted `first_name` / `last_name` / `public_id` are seeded from Auth `user_metadata` when the profiles value is missing or null (picture-only create/repair).
