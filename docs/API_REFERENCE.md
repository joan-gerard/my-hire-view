...
- **Primary CV:** when `cv_type` is `"primary"`, `primary_cv_id` must reference a row in the caller’s `primary_cvs` library; `cv_url` is resolved from that library row (client may send the same URL for convenience). **DB backstop (B3-042):** trigger `applications_primary_cv_same_user` rejects inserts/updates where `primary_cv_id` is not owned by the same `user_id`; trigger `primary_cvs_user_id_immutable` rejects any change to `primary_cvs.user_id` (keeps the id-only FK with `ON DELETE SET NULL` — a composite FK cannot use `SET NULL` without also nulling `user_id`). Migration `027` also **quarantines** any pre-existing cross-user rows (null FK, `status = draft`, scrub `cv_url` to a per-row sentinel so tailored uniqueness from `026` is preserved) so public pages cannot keep serving a stolen PDF URL.
...
- Returns **201** with the created row (`data`) and `public_id` (ensured on the profile during create) so the client can redirect to `/view/{public_id}/{slug}` without relying on stale client state.
- **Draft by default (F16-050):** omitted `status` inserts as `"draft"` (not publicly visible). The create form’s **Save & Preview** sends `status: "draft"` and redirects to `/view/{publicId}/{slug}` when `public_id` is present on the response (or already known client-side); otherwise it returns to `/admin`. **Publish** (dashboard or draft banner) sets `status: "active"`. Explicit `status: "active"` on create still works for callers that need an immediately live app.
...
Owner-only fetch for the edit page and draft-preview CV retry. Same `cv_exists` behaviour as the public slug GET. When `show_profile_picture` is true, also attaches display-only `profile_picture_url` from the owner's profile (cache-busted), matching public share / draft preview SSR.

- **Auth:** Required
- **Rate limit:** Default (60/min)
- **Success:** `200` `{ data: Application & { cv_exists?: boolean; profile_picture_url: string | null } }`
...
- Library UI ignores stale GET responses (so a slow initial load cannot overwrite a later upload/delete refresh) and disables upload until the first load completes. Closing the Manage-library modal marks that instance unmounted and invalidates its in-flight GET so upload/delete follow-up refreshes cannot call `onLibraryChange` after a newer reopen.
...
Derive a slug from company/role (and optional name-in-URL rules) via `reserveBaseSlug`. Generated slugs are clamped to **128** characters (same limit as `validateSlugFormat`). When name-in-URL is set and the combined slug would exceed 128, the **name segment is preserved** and company/role is truncated first (`buildSlug`). Returns **409** if that exact slug is already taken **for the current user** (no numeric suffix). Optional `excludeId` ignores the current row when editing. Slugs are unique per user (`UNIQUE (user_id, slug)`), not globally.
...
- **Errors:** `400` invalid/missing fields or malformed JSON; `401` bad credentials (generic message); `413` body too large; `429`; `500` no session or unexpected Auth failure
...
- **Success:** `200` `{ success: true, requiresConfirmation: false }` with cookies when a session is created immediately; or `200` `{ success: true, requiresConfirmation: true }` when email confirmation is required. **Duplicate / already-registered emails** return the same `200` + `requiresConfirmation: true` body **and** copy any cookies written during `signUp` (e.g. PKCE verifier) so `Set-Cookie` cannot enumerate accounts (F1-040).
