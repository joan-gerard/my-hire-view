...
- **Client Save UX (F8):** `ApplicationForm` maps upload **400** / **401** / **409** / **429** / **5xx** (and network failures) via `messageForUploadFailure` / `messageForUploadNetworkError`; Save shows **Uploading…** during `POST /api/upload` then **Saving…**; file identity for cache/idempotency is a **content SHA-256** signature; the same idempotency key is reused on network/**5xx** retry, and rotated on **409** or when the selected file content changes (`resolveCvUploadIdempotencyKey` — F8-063).
...
- **Client Save UX (F8):** `ProfileForm` and `ProfilePictureModal` share `uploadProfilePictureFile` for request + friendly error mapping; Save shows **Uploading…** during `POST /api/upload/profile-picture`, then **Saving…**.
