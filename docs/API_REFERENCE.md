...
- After a successful delete, the library UI refreshes the list **then** shows a still-referenced warning when `applications_affected` > 0 (so the refresh cannot wipe the message).
