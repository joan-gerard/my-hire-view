...
Replacing or clearing the picture deletes the previous Storage object after a successful profile upsert **only when the picture URL changed**. When that happens, `PUT /api/profile` re-reads the live `profile_picture_url` and, if this request is still current, purges other objects in the user’s folder (keeping the committed path). Name-only saves do not sweep Storage (avoids deleting a freshly uploaded but not-yet-committed other-extension file). A concurrent PUT that already won skips cleanup rather than deleting the winner’s avatar. Upload itself does not purge. Partial cleanup failures currently return `warnings` to the client (F9); after-launch `L11-114` adds an orphan-avatar cleanup cron and drops those user-facing delete/sweep warnings (server logs only).
