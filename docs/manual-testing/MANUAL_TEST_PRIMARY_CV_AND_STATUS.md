...
Use this after applying migrations **`021_application_status_and_archived_at.sql`**, **`022_master_cvs.sql`**, **`024_primary_cvs_rename.sql`**, **`027_applications_primary_cv_same_user.sql`**, and **`028_primary_cvs_library_cap.sql`**. Prefer a test account with R2 env vars configured (`.env.local`). See [CV_REUSE_AND_STORAGE.md](../retrospectives/CV_REUSE_AND_STORAGE.md).
...
- [ ] (F13-032) With 5 primaries already, a further `INSERT` into `primary_cvs` is rejected by trigger `primary_cvs_library_cap` (`primary_cvs_library_cap_exceeded`)
...
All applicable boxes pass with migrations `021`–`028` applied, and `pnpm test:ci` is green.
