...
| `/how-it-works`     | How it Works marketing page: compact `MarketingHero` (image + `PageHeroContent` with title/subtitle), then main content. Uses same `MarketingHeader` as `/`.                                                                                                                                                                                        | No   |
...
│   ├── pdf/                    # PDFViewer
│   ├── public/                 # ApplicationPageHeader, EmailCaptureForm, FAQSection, FinalCTASection, Footer (→ ViewPageFooter), HomeHeroContent, HowItWorksSection, MarketingHero (reusable: backgroundImage + children + optional imageCredit), MarketingHeader, PageHeroContent (reusable title + subtitle for How it Works / Pricing / Blog), ProblemSection, SocialProofSection, SolutionSection, ViewPageFooter
