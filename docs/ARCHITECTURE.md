...
| `/api/applications/[slug]/view` | POST                   | Increment `view_count` for slug (owner views not counted)                | No                                                |
...
- **Table: `applications`**
  - `id` (UUID, PK), `slug` (unique), `company`, `role`, `cv_url`, `video_url`, `description`, `created_at`, `updated_at`, `view_count`, `download_count` (CV downloads from public page; owner not counted), `user_id` (FK to `auth.users`), `is_active` (default true; archiving = soft hide).
