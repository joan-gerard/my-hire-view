...
| `/pricing`          | Pricing stub (real tiers = E3): `FixedBackgroundHero` + scroll content. Uses same `MarketingHeader` as `/`.                                                                                                                                                                                                                      | No   |
...
- **Table: `applications`**
  ...
  - **Same-user ownership (B3-042):** trigger `applications_primary_cv_same_user` ensures `primary_cv_id` belongs to the same `user_id` as the application (API also checks; DB is the backstop). Composite FK was avoided so `ON DELETE SET NULL` does not clear `user_id`.
...
│   │   └── pricing/            # Pricing page (stub until E3)
