...
│   ├── pdf/                    # PDFViewer
│   ├── public/                 # ApplicationPageHeader, EmailCaptureForm, FAQSection, FinalCTASection, Footer (→ ViewPageFooter), HomeHeroContent, HowItWorksHero (How it Works page: content above + fixed image), HowItWorksScrollSection (content that scrolls over fixed hero image), HowItWorksSection (see public/how-it-works/ for StepLabel, StepCard, useHowItWorksObservers, constants; on home page dark mode is triggered when ProblemSection enters viewport via LandingPageSections), LandingPageSections (client wrapper: composes sections and passes ProblemSection ref to HowItWorksSection for dark-mode trigger), MarketingHero (reusable: backgroundImage + children + optional imageCredit), MarketingHeader, PageHeroContent (reusable title + subtitle for Pricing / Blog), ProblemSection, SocialProofSection, SolutionSection, ViewPageFooter
