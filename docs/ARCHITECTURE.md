...
│   ├── pdf/                    # PDFViewer
│   ├── public/                 # ApplicationPageHeader, EmailCaptureForm, FAQSection, FinalCTASection, Footer (→ ViewPageFooter), HomeHeroContent, HowItWorksHero (How it Works page: content above + fixed image), HowItWorksScrollSection (content that scrolls over fixed hero image), HowItWorksSection, MarketingHero (reusable: backgroundImage + children + optional imageCredit), MarketingHeader, PageHeroContent (reusable title + subtitle for Pricing / Blog), ProblemSection, SocialProofSection, SolutionSection, ViewPageFooter
