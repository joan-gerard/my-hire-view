...
- **Brand colors:** Defined in `app/globals.css` (warm light theme). Background `--background` (#f6f0e9), secondary `--secondary-background` (#fcf6ef), primary text/buttons `--brand-primary` (#2b1809) with white text, secondary buttons `--brand-secondary` (#f0e7dd), teal accent `--brand-accent-1` / `--brand-accent-2` for homepage CTAs and positive product actions. Product status tokens (`--status-success-*`, `--status-draft-*`, `--status-warning-*`, `--status-danger-*`) keep `/admin` and `/view` aligned with the same palette.
...
- **Images:** Marketing stills live in `public/` as compressed WebP (`hero-image.webp`, `remote-work-2.webp`, `solution-2.webp`, `solution-1-1.webp`, `customer-service-250x250.webp`) and are served via Next.js `Image`. Unused SVGs remain in `public/images/` (legacy brief assets). Hero and How It Works **videos** are served from R2 (`marketing/*.mp4`); see [PDF_AND_R2.md](PDF_AND_R2.md).
