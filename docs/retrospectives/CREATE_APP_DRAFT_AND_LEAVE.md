...
- Successful **Save & Preview**: disarm leave-confirm (`disarmForNavigation`) **before** `router.push` to the share URL so sentinel `history.back()` cannot cancel the preview redirect (F16).
