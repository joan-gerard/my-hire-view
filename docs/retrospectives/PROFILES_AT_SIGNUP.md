...
| Rare `public_id` unique collision | Re-select by `user_id`; if still missing, regenerate `public_id` and sync Auth metadata |
| Invalid Auth `public_id` metadata | `publicIdFromUserMetadata` / `createInitialProfile` validate with `isValidPublicId` or regenerate |
