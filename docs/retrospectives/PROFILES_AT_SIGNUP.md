...
- `PUT /api/profile` still upserts (can create a row if somehow missing). Omitted names/`public_id` are seeded from Auth metadata when the profiles value is missing or null so picture-only PUTs succeed (C3-026).
...
| Picture-only PUT needs names | Names exist on the row from signup → merge succeeds. If the row is missing or has null names, omitted names/`public_id` are seeded from Auth metadata (C3-026). |
