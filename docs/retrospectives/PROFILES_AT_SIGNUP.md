...
| Invalid Auth `public_id` metadata | Pass raw metadata into `createInitialProfile`; validate with `isValidPublicId` or regenerate and reconcile Auth by reading Auth. Existing rows with an invalid/empty `public_id` are repaired via shared `repairProfilePublicId` (conditional update of **`public_id` only** + re-read — never overwrites names). Read-only resolve returns **null** when a profiles row exists but is invalid (no Auth fallback). Preferred metadata ids owned by another user are rejected before use. Create-path upserts retry collisions up to the shared attempt bound. |
