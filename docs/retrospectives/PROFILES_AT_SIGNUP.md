...
- `PUT /api/profile` still upserts (can create a row if somehow missing). Omitted names/`public_id` are seeded from Auth metadata when the profiles value is missing or blank so picture-only PUTs succeed; resolved names still enforce the 100-character max (C3-026).
...
| Picture-only PUT needs names | Names exist on the row from signup → merge succeeds. If the row is missing or has blank names, omitted names/`public_id` are seeded from Auth metadata; over-long seeds still **400** (C3-026). |
